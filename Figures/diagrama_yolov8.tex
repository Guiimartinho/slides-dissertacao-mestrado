...
\begin{tikzpicture}[
    ...
    % Estilos das labels
    sectionlabel/.style={font=\footnotesize\sffamily\bfseries, text=labelcolor},
    sizelabel/.style={font=\tiny\sffamily, text=sublabelcolor},
]
...
\begin{scope}[shift={($(output) + (4.5, 2.5)$)}]
    \node[font=\footnotesize\sffamily\bfseries, text=labelcolor] at (0, 1.2) {Legenda:};

    \node[block, fill=backbonecolor, minimum width=1.0cm, minimum height=0.4cm] at (0, 0.8) {\tiny CBS};
    \node[font=\tiny\sffamily, text=labelcolor, anchor=west] at (0.6, 0.8) {Conv+BN+SiLU};

    \node[block, fill=c3color, minimum width=1.0cm, minimum height=0.4cm] at (0, 0.4) {\tiny C2f};
    \node[font=\tiny\sffamily, text=labelcolor, anchor=west] at (0.6, 0.4) {CSP Bottleneck};

    \node[block, fill=headcolor, minimum width=1.0cm, minimum height=0.4cm] at (0, 0.0) {\tiny Head};
    \node[font=\tiny\sffamily, text=labelcolor, anchor=west] at (0.6, 0.0) {Cls + Reg separados};

    \draw[arrow] (-0.4, -0.4) -- (0.2, -0.4);
    \node[font=\tiny\sffamily, text=labelcolor, anchor=west] at (0.4, -0.4) {Fluxo principal};

    \draw[skiparrow] (-0.4, -0.7) -- (0.2, -0.7);
    \node[font=\tiny\sffamily, text=labelcolor, anchor=west] at (0.4, -0.7) {Skip connection};
\end{scope}
